...
普通查询与迁移使用不同超时，避免大表回填被普通 30 秒限制打断。详见 [数据库迁移](../db/MIGRATIONS.md)。
...
- 以下组合文件超过 800 行重构信号：`Reader.vue` 与 Reader 的详情/书库/首页、`server-pg.js`、`telegram-bot.js`、`po18-crawler.js`、`docker/control-panel.js`、Admin `BooksView.vue`、本地书库上传工具。新增能力应进入现有 component/mixin/service/route，而不是继续扩大组合根。
- `chinese-convert.js` 主要体积来自静态转换表，适合按生成数据与运行算法拆分；Reader 当前该 chunk 构建后仍超过 500 KiB，后续应评估异步加载或数据压缩，而不是简单调高警告阈值。
