 ...
 * [OUTPUT]: 提供可信来源、旧登录代理缺失来源头和跨站/受保护写入拒绝场景的 CSRF 自动化回归断言
...
test("CSRF protection preserves legacy login with an unauthenticated stale Session cookie", () => {
    const middleware = createCsrfProtection({ env: { NODE_ENV: "production" } });
    for (const path of ["/admin-api/auth/login", "/reader-auth/login", "/reader-auth/telegram"]) {
        const login = run(
            middleware,
            request({
                path,
                headers: {
                    host: "127.0.0.1:3100",
                    cookie: "po18_upload_admin_pg=stale-session"
                },
                session: {}
            })
        );
        assert.equal(login.continued, true, `${path} should accept the legacy proxy request`);
    }

    const authenticatedRelogin = run(
        middleware,
        request({
    ...
    assert.equal(authenticatedRelogin.res.statusCode, 403);
    ...
    assert.equal(registration.res.statusCode, 403);
});
