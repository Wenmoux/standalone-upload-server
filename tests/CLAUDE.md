...
- `admin-ui-contract.test.js`: Admin 导航、会话恢复、无障碍公共控件、章节整理全量明细与结果定位、非阻塞输入及高密度工作区分层契约。
...
- `bot-entry-handlers.test.js`: Bot 账户/经济/导出/PikPak 处理器的私聊续接、PEER_ID_INVALID 降级、基础/工坊 EPUB 图片预览与回调、实际导出耗时、发送后结算、权限及文案契约。
...
- `bot-runtime-modules.test.js`: Bot 业务组合根/进程生命周期装配、Telegram 文本/预览图片幂等编辑、EPUB 自定义任务持久化、搜索缓存、短期书评/管理员广播草稿、批量投递与依赖边界。
...
- `bot-task-runtime.test.js`: 持久任务领取、执行、续租、fencing token 回写、完成和最终导出失败耗时反馈语义。
...
- `control-panel.test.js`: Setup 配置/诊断/页面/路由边界、Token、导入导出、脱敏、共享设计令牌、状态和版本契约。
...
- `epub-preview.test.js`: Telegram/QQ 共享 EPUB 三联实时预览的 PNG、组件名称投影、配置差异和有界缓存契约。
...
- `qq-bot.test.js`: QQ AppSecret 脱敏、平台/标签/实时缓存范围、签到、模板库基础/工坊图片预览与导出、移动端可折叠代码块/纯文本降级、主面板/状态卡层级、紧凑内嵌按钮、Gateway 事件归一化、Access Token 与分阶段重试富媒体上传契约。
