 ...
 * [OUTPUT]: 提供导出错误分类、私聊不可达识别、重试语义与面向用户消息的自动化回归断言
...
test("export errors classify common failures", () => {
    ...
    assert.equal(
        classifyExportError(asExportError(EXPORT_ERROR_CODES.PRIVATE_CHAT_REQUIRED, "blocked")).code,
        EXPORT_ERROR_CODES.PRIVATE_CHAT_REQUIRED
    );
    assert.equal(classifyExportError(new Error("Bad Request: PEER_ID_INVALID")).code, EXPORT_ERROR_CODES.PRIVATE_CHAT_REQUIRED);
    assert.equal(isPrivateChatUnavailableError(new Error("Bad Request: PEER_ID_INVALID")), true);
    assert.equal(isPrivateChatUnavailableError(Object.assign(new Error("Bad Request"), { code: "PEER_ID_INVALID" })), true);
    assert.equal(asExportError(EXPORT_ERROR_CODES.PRIVATE_CHAT_REQUIRED, "peer").retryable, false);
