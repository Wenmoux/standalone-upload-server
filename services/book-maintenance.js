...
function createBookMaintenanceService(options = {}) {
    ...
    const recordEvent = options.recordEvent || (async () => null);

    async function stalePo18BooksPreview(dbQuery = query) {
        ...
        const summary = await dbQuery(
            `WITH targets AS (
             ...
             SELECT COUNT(*)::int metadata_count,
                    COUNT(DISTINCT book_id)::int book_count,
                    COALESCE((SELECT COUNT(*)::int FROM chapter_cache c WHERE c.book_id IN (SELECT book_id FROM targets)), 0)::int chapter_count
             FROM targets`,
            params
        );
    ...
    async function cleanupStalePo18Books({ actor = "admin" } = {}) {
