/**
 * [INPUT]: 依赖 Node crypto、版本化主密钥环和待存储的 PO18 Cookie/账户及 QQ AppSecret 凭证字段
 * [OUTPUT]: 对外提供通用字符串/JSON 凭证加解密器、密钥环解析、PO18 存量轮换函数及密文标识常量
 * [POS]: services 的外部凭证边界，为数据库配置和账户字段提供可轮换的版本化 AES-GCM 保护
