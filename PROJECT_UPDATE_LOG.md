...
- Reader 代理在连接 3100 时保留浏览器访问的公网 Host/协议；CSRF 对浏览器明确标记为 `same-origin` 的请求提供安全回退，同时继续拒绝 `cross-site` 写入，不需要开启通配 CORS 或关闭来源校验。
- 增加代理头和反向代理 Host 改写场景回归，锁定会话 Cookie 与登录来源语义。

## 2026-07-14：番茄正文缓存一键清理

- 新增 `npm run db:purge-fanqie-cache`，事务化删除 `fanqie`、`fq`、`tomato` 的正文缓存，保留全部书籍元信息，并以 JSON 返回删除章节数和去重书籍 ID 数组。
- 平台为空的历史缓存只在元信息能唯一证明属于番茄时清理；同号其他平台缓存受保护。清理期间以表级写锁隔离并发上传，失败会完整回滚。
- Docker 的 server-pg 与 app 运行阶段均收录该维护脚本，容器内无需手工加载 `/config/app.env`。
