...
- 新增“样式二 · 原书复刻”，按参考 EPUB 的标题页、制作说明、书籍信息、分卷图、正文章头和嵌套目录结构生成；只有章节数据包含非空真实分卷时才生成分卷页，不再自动补“正文”。
- 后台 EPUB 编辑器支持样式一、样式二的五类页面实时预览和完整样式 CSS 展示；样式二背景资源以登录态加载后嵌入预览，简介页不再因图片鉴权失败显示白底，并增加文字对比度。
- 样式二预置文本、字体族和追加 CSS 可直接配置；10 个图片资源位可独立替换或恢复，并显示推荐尺寸与当前尺寸。
- 自定义样式图片写入 `/config/epub-style2/`，重建容器后仍保留；未上传的资源继续使用内置原图。
- Bot 的 EPUB 导出改为先选择“样式 1 / 样式 2”，选择后再进入冷却、任务队列和扣费流程；群聊转私聊会保留所选样式。
