...
## 2026-07-24：EPUB 章节标题保真

- EPUB 不再按章节数组位置自动生成“第 X 章”；只有来源标题自身含章号标签时才拆分排版。
- “数字 + 章节名”等非“第 X 章”标题在四套可选样式和兼容样式中完整显示，目录继续使用原始标题，TXT 导出逻辑不受影响。
- 四套 XHTML 章页模板改为条件章号行，避免未提供源章号时留下空白行；后台预览、样式契约和回归测试同步更新。
...
- Bot 启动及搜索前按一分钟缓存读取 `/reader-api/platforms`，后台新增的实际平台可直接作为命令后缀，不再被误拼进关键词或标签。
- Bot 标签语法扩展为 `#标签`、`＃标签`、`tag:`、`t:`、`标签：`、`分类：`；普通搜索显式包含仅有元信息的书，热门与随机仍只推荐已有正文缓存的书。
- 热门和词云文案明确区分全站热搜与指定平台标签，避免把全站热词误报为单个平台统计；平台别名、动态配置、标签组合和未缓存元信息回归同步补齐。

## 2026-07-22：EPUB 丹青云卷样式四

- 新增 `style4`“丹青云卷”，按参考 EPUB 复刻长屏封面语义、制作说明、书籍信息、简介、双原字体、彩墨竖排分卷、双色章题和嵌套目录。
- 生成器新增可选独立书籍信息页，顺序固定为封面、制作说明、书籍信息、简介，再进入真实分卷和章节；其他样式不受影响。
- 参考 EPUB 的正文章头图含固定内容，按产品要求完全排除：资源不进入仓库、manifest 和正文 XHTML，正文直接从双色章题开始。
- Admin 增加样式四的封面、说明、信息、简介、分卷和正文同构预览；Bot 直选、API、Docker 构建输入和回归测试同步扩展。

## 2026-07-22：EPUB 空门夜雨替换旧样式三

- 内部 ID `style3` 保持不变，显示名由“疏影横斜”改为“空门夜雨”，既有后台和 Bot 默认样式配置无需迁移。
- 按新参考 EPUB 替换样式三：打包完整楷体与粗宋字体，恢复正文、引用、脚注、轻灰说明框、40% 顶距下划线分卷标题和居中粗宋章题。
- 删除旧梅影、数字字体和三张分卷底图；真实分卷改为普通独立 XHTML 与嵌套目录，不再使用 SVG、整页图片或 `duokan-page-fullscreen` 分卷属性。
- 后台五类页面预览、Bot 直选按钮、当前 API/README、Docker 构建输入和回归测试同步更新。

## 2026-07-17：章节结构整理与后台入口收口

- 数据质量页把同名分卷清理并入章节顺序整理：同一本书内仅比较 `is_volume=true` 且非空的卷名，忽略首尾空格后同名时保留顺序最早的一条，删除后续重复项；不同卷名不受影响。
- 统一操作先预览再确认，执行时先删除重复分卷，再连续重排受影响书籍，并作为单个 `chapters_repair_order` 任务记录；结果表列出实际改动书籍、移除卷名和重排章节数。
- 预览取消后台默认 50 本截断，明确区分“受影响书籍”“待删除重复分卷记录”“重复 order 章节”；确认弹窗按序号列出全部命中书籍，不再以样例截断明细。
- 执行由逐章双阶段更新改为按全部目标书集合锁定、批量删除和两次集合重排；完成后页面自动滚动到全部实际改动书籍，避免长时间等待后找不到结果。
- 独立重复分卷 API 与历史 `chapters_cleanup_duplicate_volumes` 重试仍保留兼容，但后台不再显示单独入口。
- 书籍页移除“清理旧 PO18”按钮及前端调用逻辑；兼容 API 暂时保留，避免历史任务重试和外部运维调用突然失效。

## 2026-07-16：章节顺序全平台唯一

- 新增 `024_chapter_order_uniqueness`：只处理存在正数重复顺序的书，按 `chapter_order → chapter_id → id` 稳定重排后，将同书正数 `chapter_order` 唯一索引扩展到全部平台。
- 迁移在重排后、建索引前显式执行延迟章节顺序约束，避免历史重复数据触发 `pending trigger events` 而阻断升级。
- 普通上传与 order-only 更新统一持有同书事务锁；目标位置冲突时以临时非正数腾位并移动受影响区间，避免唯一索引错误和正文误覆盖。
- 后台重复检测改为整书跨平台扫描，确认修复采用两阶段更新，避免重排过程中与尚未移动的旧顺序碰撞。
...
## 2026-07-16：Reader OpenCC 1.3 兼容修正

- 繁转简冻结兜底映射补充 `擡 -> 抬`，修复锁定 Node 20/`opencc-js@1.3.0` 时“抬头”回归失败的问题。
- Node 20 完整覆盖率测试保持 445 项、444 通过、1 跳过、0 失败；转换维护说明同步记录该版本差异。

## 2026-07-16：历史章节目录名一次性清洗脚本增强

- `scripts/clean-chapter-titles.js` 继续保持一次性手工入口：默认 dry-run，`--apply` 才写库，并在单个 PostgreSQL 事务中提交，避免中途失败留下半批结果。
- 清洗顺序固定为：空白规范化 → 已确认括号/未闭合尾注 → 用户自定义正则 → 标点与首尾连接符整理；支持 `--volumes-only` 只预览或清洗分卷行。
- 清洗后为空恢复原标题；脚本只更新 `chapter_cache.title` 和 `updated_at`，不接入启动、上传或定时任务。

## 2026-07-16：Docker 控制面板边界拆分

- 将 1,221 行控制面板拆为 744 行鉴权/路由组合根、302 行纯页面渲染器和 275 行只读运行诊断服务；配置写入、外部探测与 HTML 不再互相持有隐式状态。
- Docker 构建上下文、模块地图、格式门禁和控制面契约测试同步纳入两个新运行文件，Setup 原有 URL、公开导出和部署入口保持兼容。
...
## 2026-07-16：繁简转换报告职责拆分

- 将 1,040 行离线报告脚本拆为 535 行分析核心、453 行纯渲染器和 86 行 CLI 组合根；正文扫描、汇总模型、HTML/JSON 展示和退出码不再耦合。
- 新增真实内置语料组合回归，直接执行生产转换器并验证 analyzer → summary → renderer 数据流，固定 `npm run test:convert` 命令与报告路径保持不变。
- Reader API 正文扫描器同步把 Markdown/HTML 展示抽到独立写入器，扫描组合根从 824 行降至约 600 行，断点与 API 请求不再依赖页面模板细节。

## 2026-07-16：Bot 进程生命周期收口

- 将命令同步、持久任务恢复、管理员广播轮询、健康服务和 Telegram 长轮询启动抽到独立 process runtime；业务组合根从 801 行降至约 740 行，只保留领域依赖注入与 update 分派。
- 结构契约锁定业务组合根不再直接创建 polling runtime，并确认生命周期层仍同时接管健康服务和永久轮询。

## 2026-07-16：PostgreSQL 集成测试分域

- 将 1,013 行真实 PostgreSQL 集成测试拆为 192 行顺序组合根、519 行平台核心用例组和 430 行业务/运维用例组；全部子测试仍共享同一迁移数据库，不引入跨文件 schema 重置竞争。
- 原 `tests/pg-flows.test.js` 入口和发布流水线命令保持不变，迁移、任务租约、结算幂等、治理清单、真实备份恢复与回滚覆盖均未删除。
- 新增全仓 800 行源码预算测试，持续扫描 JS/Vue/CSS/Less/TS/Python/Shell/PowerShell，明确排除历史备份、依赖和生成产物，防止拆分成果回潮。

## 2026-07-16：本地书库上传工作台拆分

- 删除从未被路由调用的旧版工作台整页模板，现用页面不再把两套 HTML、CSS 和浏览器脚本同时塞在服务端字符串中。
- 独立拆出基础视觉、书籍/章节工作区视觉和浏览器控制器；服务端文件从约 2,400 行降至约 550 行，只保留扫描/编辑/上传 API、静态资源路由和安全默认值注入。
- 新增真实 HTTP 资源回归，验证页面引用、CSS/JS 响应、默认值转义及脚本注入防护；四个工作台源码文件均纳入根级 Prettier 门禁并保持在 800 行以内。
- 继续把千行导入脚本拆为纯扫描核心、manifest/上传服务和 210 行 CLI 组合根；旧命令与 CommonJS 导出保持兼容，解析、传输和交互不再共享隐式状态。
- Docker digest 回拉冒烟暴露 Admin 登录响应与 PostgreSQL Session 落库之间的偶发时序窗口；登录路由现在显式等待 `session.save()` 后再返回 200，避免紧接登录的当前用户或退出请求偶发 401。

## 2026-07-16：Reader 设置面板与样式边界拆分

- 阅读设置抽屉从 `Reader.vue` 提取为独立设置面板；面板只接收设置、主题、音色和章头预览状态，并通过语义事件提交变更，设置持久化与 TTS 引擎编排仍由原领域 mixin 负责。
- 面板控制器、面板样式与正文样式分别进入独立文件，避免把千行页面搬成另一个千行组件；`Reader.vue` 从约 2,000 行降到 400 行以内，设置面板模板也保持在 800 行阈值以内。
- 增加结构契约测试，锁定抽屉归属、事件转发、样式隔离和文件规模；同步修正架构文档中过时的 OpenCC 体积归因。
