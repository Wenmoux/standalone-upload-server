...
## 2026-07-13：EPUB 样例结构精确对齐

- 对照《家弑服务》样例的 OPF、XHTML、CSS 与图片尺寸，补齐封面页和“疏影横斜”真实分卷页的 `duokan-page-fullscreen` spine 语义；分卷继续只来自源数据并在 NCX 中作为章节父节点。
- 原封面继续写入 EPUB cover 元数据，另用 Resvg 自动生成 `1080×2400` 的 `cover~slim.png`，以模糊延展背景和完整前景适配长屏，替代纯黑上下留空。
- “疏影横斜”分卷改为独立全屏 XHTML/SVG 留白画布，动态排版卷序和卷名；制作说明改为样例的 20% 顶距、浅灰边框、60% 灰字说明框，Admin 预览同步使用同构 DOM/CSS。
...
- 新增文艺简约样式“疏影横斜”（内部 ID `style3`）：参考《家弑服务》EPUB 的大片留白、黑白层级、居中章序和分部目录，改用可适配任意书名的暖白纸面、淡墨梅影与系统宋体回退，不复制原书带固定文字的分部图或裁剪字体。
...
- 老二次元预置文本、字体族和追加 CSS 可直接配置；10 个图片资源位可独立替换或恢复，并显示推荐尺寸与当前尺寸。
