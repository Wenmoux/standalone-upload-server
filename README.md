...
分卷页只根据真实非空分卷数据生成，不会自动补“正文”。“空门夜雨”使用纯排版分卷；“丹青云卷”使用彩墨长屏分卷并增加独立书籍信息页，但明确不打包或引用固定正文章头图。可见封面同时生成长屏适配图，原图仍用于书库封面元数据。EPUB 保留来源章节标题：只有原标题自带“第 X 章”时才拆分章号，不会按章节位置自动补写；正文首行与章节标题完全等价时会自动去重。开发说明见 [EPUB 样式文档](bot/epub-styles/README.md)。
