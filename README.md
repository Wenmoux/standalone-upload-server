...
- 支持 QQ 单聊和群聊 @ 搜索，书名、作者、`#标签`、翻页与序号选书的内容层级与 Telegram 书卡一致；普通搜索只返回已有正文缓存、可实际下载的结果。
- 支持 TXT、四种成品 EPUB 样式及 288 种组件组合的模板工坊；Telegram/QQ 在确认前共享简介/分卷/正文三联 PNG 实时预览，导出继续使用现有缓存、用户额度、币种结算和 200 MB 内 QQ 官方富媒体分片上传。
...
分卷页只根据真实非空分卷数据生成，不会自动补“正文”。“空门夜雨”使用纯排版分卷；“丹青云卷”使用彩墨长屏分卷并增加独立书籍信息页，但明确不打包或引用固定正文章头图。可见封面同时生成长屏适配图，原图仍用于书库封面元数据。EPUB 保留来源章节标题：只有原标题自带“第 X 章”时才拆分章号，不会按章节位置自动补写；正文首行与章节标题完全等价时会自动去重。Telegram 与 QQ 可直接选择成品模板、调整基础开关，或在模板工坊中组合章题、分卷、简介和装饰；配置会随持久任务和群聊转私聊续接保留。开发说明见 [EPUB 样式文档](bot/epub-styles/README.md)与 [EPUB 模板库](docs/EPUB_TEMPLATE_LIBRARY.md)。
