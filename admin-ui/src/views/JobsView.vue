    ...
    <section>
        <div class="view-head">
            ...
            <div class="row-actions">
                <span class="sub" aria-live="polite"
                    >{{ autoRefreshing ? "有任务运行，自动刷新中" : "按需刷新" }} · 更新于 {{ lastUpdated || "-" }}</span
                >
        ...
        <section class="panel">
            <div class="section">
                ...
                <div class="dashboard">
                    <StatCard label="全部任务" :value="number(overview.total || 0)"
                        >System Jobs {{ overview.available ? "已启用" : "未启用" }}</StatCard
                    >
                    <StatCard label="排队 / 运行" :value="`${number(statusCount('queued'))} / ${number(statusCount('running'))}`"
                        >Queued / Running</StatCard
                    >
        ...
        <section class="panel">
            <div class="section">
                ...
                <DataTable :columns="columns" :rows="rows" :loading="loading" table-class="jobs-table" empty-text="暂无任务">
                    ...
                    <template #cell-actions="{ row }">
                        <div class="compact-actions">
                            ...
                            <button
                                class="secondary"
                                type="button"
                                :disabled="!canRetry(row) || retryingId === row.id"
                                @click="retryJob(row)"
                            >
                            ...
                            <button
                                ...
                                type="button"
                                :disabled="!canCancel(row) || cancelingId === row.id"
                                @click="cancelJob(row)"
                            >
                ...
                <div class="pager">
                    ...
                    <button class="secondary" type="button" :disabled="page >= totalPages || loading" @click="load(page + 1)">
                        下一页
                    </button>
                </div>
            </div>
...
const retryableTypes = new Set([
    ...
    "chapters_cleanup_duplicate_volumes",
    "po18_crawler_run"
]);
