            ...
            <div class="section">
                ...
                <DataTable :columns="columns" :rows="rows" :loading="loading" empty-text="暂无用户">
                    ...
                    <template #cell-account="{ row }">
                        ...
                        <template v-if="row.telegram_id"
                            ><br /><small>TG {{ row.telegram_id }} {{ row.telegram_username || "" }}</small></template
                        >
                    </template>
                    <template #cell-membership="{ row }">
                        <span v-if="row.membership_permanent" class="tag success">永久</span>
                        <span
                            v-else-if="row.membership_expires_at"
                            class="tag"
                            :class="new Date(row.membership_expires_at) > new Date() ? '' : 'warn'"
                        >
                    ...
                    <template #cell-library="{ row }">
                        <div class="tag-row">
                            <span class="tag" :class="row.library_access ? 'success' : 'danger'">{{
                                row.library_access ? "允许" : "禁用"
                            }}</span>
                    ...
                    <template #cell-scholar="{ row }">
                        ...
                        <small>经验 {{ number(row.scholar_exp) }}，距下级 {{ number(row.scholar?.exp_to_next ?? 0) }}</small
                        ><br />
                        <small
                            >今日付费书免费 {{ number(row.free_exports_today) }}/{{
                                number(row.daily_free_exports || row.scholar?.daily_free_exports || scholarLevel(row))
                            }}
                            本</small
                        ><br />
                    ...
                    <template #cell-time="{ row }"
                        >创建 {{ time(row.created_at) }}<br /><small>登录 {{ time(row.last_login_at) }}</small></template
                    >
                    <template #cell-actions="{ row }">
                        <div class="row-actions">
                            <button class="secondary" type="button" @click="openEditor(row)">改</button>
                            <button class="secondary" type="button" @click="grant(row)">会员授权</button>
                            <button
                                v-if="user?.role === 'owner'"
                                class="secondary"
                                :class="{ danger: row.is_admin }"
                                type="button"
                                :disabled="adminBusy === row.id"
                                @click="toggleAdmin(row)"
                            >
                                {{ adminBusy === row.id ? "处理中..." : row.is_admin ? "取消管理员" : "设为管理员" }}
                            </button>
...
function openEditor(row = null) {
    ...
    userFields.value = row
        ...
        : [{ key: "username", label: "用户名" }, { key: "password", label: "密码", type: "password" }, ...baseFields];
    modal.model = {
...
async function toggleAdmin(row) {
    const nextAdmin = !row.is_admin;
    const label = row.nickname || row.username || row.id;
    const confirmation = await confirmAction({
        title: nextAdmin ? "设为 Reader/Bot 管理员" : "取消 Reader/Bot 管理员",
        message: nextAdmin
            ? `将授予 ${label} Bot 发币、免开通导出和管理员日报接收资格。`
            : `将取消 ${label} 的 Bot 发币、免开通导出和管理员日报接收资格。`,
        confirmLabel: nextAdmin ? "设为管理员" : "取消管理员"
    });
    if (!confirmation.confirmed) return;
    adminBusy.value = row.id;
    try {
        await api(`/admin-api/users/${row.id}/admin`, {
            method: "PATCH",
            body: JSON.stringify({ is_admin: nextAdmin, reason: confirmation.reason })
        });
        await load();
        toast(nextAdmin ? "已设为 Reader/Bot 管理员" : "已取消 Reader/Bot 管理员");
    } catch (err) {
        toast(err.message || String(err));
    } finally {
