<template>
    <section>
        ...
        <section class="panel">
            <div class="section">
                <div class="toolbar">
                    ...
                    <label class="field">
                        ...
                        <select v-model="filters.platform">
                            ...
                            <option v-for="item in platformOptions" :key="item.value" :value="item.value">{{ item.label || item.value }}{{ item.count ? ` (${number(item.count)})` : "" }}</option>
                        </select>
                    </label>
                    <label class="field">
                        <span>排序</span>
                ...
                <div v-if="categoryOptions.length" class="filter-row category-filter-row">
                    <button v-for="row in categoryOptions" :key="row.category" class="secondary" :class="{ active: filters.category === row.category }" type="button" @click="toggleCategory(row.category)">{{ row.category }} · {{ number(row.count) }}</button>
                </div>
                ...
                <DataTable :columns="bookColumns" :rows="books" :loading="loading" :sort-value="sortValue" empty-text="没有找到书籍" table-class="books-table" @sort="setSort">
                    <template #cell-select="{ row }">
                        <input class="row-check" type="checkbox" :checked="isSelected(row)" :aria-label="`选择《${row.title || row.book_id}》`" @change="toggleRow(row)" />
                    </template>
                    <template #cell-book_id="{ row }"
                        ><code>{{ row.book_id }}</code></template
                    >
                    <template #cell-title="{ row }">
                        <div class="book-title-cell">
                            <strong>{{ row.title || row.book_id }}</strong>
                            <small
                                ><code>{{ row.book_id }}</code></small
                            >
                        </div>
                    </template>
                    ...
                    <template #cell-counts="{ row }">
                        <div class="metric-stack">
                            <span
                                >章节 <strong>{{ number(row.total_chapters ?? row.subscribed_chapters ?? 0) }}</strong></span
                            >
                            <span
                                >缓存 <strong>{{ number(row.cache_count) }}</strong></span
                            >
                        </div>
                    </template>
                    <template #cell-heat="{ row }">
                        <div class="metric-stack">
                            <span
                                >人气 <strong>{{ number(row.total_popularity) }}</strong></span
                            >
                            <span class="like"
                                >喜欢 <strong>{{ number(row.like_count) }}</strong></span
                            >
                            <span class="dislike"
                                >不喜 <strong>{{ number(row.dislike_count) }}</strong></span
                            >
                        </div>
                    </template>
        ...
        <section v-if="currentBookId" class="panel">
            <div class="section">
                ...
                <DataTable :columns="chapterColumns" :rows="chapters" :loading="chaptersLoading" empty-text="这本书还没有章节缓存">
                    ...
                    <template #cell-chapter_id="{ row }"
                        ><code>{{ row.chapter_id }}</code></template
                    >
        ...
        <FormModal :open="bookModal.open" :title="bookModal.title" :model="bookModal.model" :fields="bookFields" :textarea-fields="bookTextareaFields" :busy="bookModal.busy" :error="bookModal.error" @close="bookModal.open = false" @save="saveBook" />
        <FormModal :open="chapterModal.open" :title="chapterModal.title" :model="chapterModal.model" :fields="chapterFields" :textarea-fields="chapterTextareaFields" :busy="chapterModal.busy" :error="chapterModal.error" @close="chapterModal.open = false" @save="saveChapter" />
    </section>
</template>
...
import { bookColumns, bookFields, bookNumericFields, bookTextareaFields, chapterColumns, chapterFields, chapterTextareaFields, defaultPlatformOptions } from "./books-config";
...
async function loadBooks(nextPage = 1) {
    ...
    try {
        ...
        const [data, categories] = await Promise.all([api(`/admin-api/books?${params}`), api("/admin-api/books/categories").catch(() => ({ rows: [] }))]);
...
async function importManifestFile(event) {
    ...
    try {
        ...
        const confirmation = await confirmAction({
            title: "导入书籍 Manifest",
            message: [`书籍：${validation.book?.title || validation.book?.book_id || "-"}`, `身份：${validation.book?.platform || "-"}:${validation.book?.book_id || "-"}`, `章节：${number(validation.chapters || 0)}`, "校验和已通过。导入会增量写入变化的数据，不会删除 Manifest 之外的章节。"].join(
                "\n"
            ),
            confirmLabel: "确认导入",
...
async function batchDeleteBooks() {
    ...
    if (!confirmation.confirmed) return;
    for (const row of selectedRows.value) {
        await api(`/admin-api/books/${row.id}?deleteMode=${deleteMode}`, {
            method: "DELETE",
...
async function deleteCurrentBookChapters() {
    ...
    if (!confirmation.confirmed) return;
    const data = await api(`/admin-api/books/${encodeURIComponent(currentBookId.value)}/chapters`, {
        method: "DELETE",
        body: JSON.stringify({ reason: confirmation.reason })
    });
    chapters.value = [];
