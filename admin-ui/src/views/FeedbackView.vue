<template>
    <section>
        ...
        <section v-show="activeTab === 'feedback'" class="panel">
            <div class="section">
                ...
                <div class="word-cloud">
                    ...
                    <button v-for="row in hotWords" :key="row.keyword" type="button" :style="wordStyle(row)" @click="navigate('books')">
                        {{ row.keyword }}<small>{{ number(row.count) }}</small>
                    </button>
                </div>
                <DataTable :columns="feedbackColumns" :rows="sortedFeedback" :loading="loading" :sort-value="sortValue" empty-text="暂无反馈" @sort="setSort">
                    ...
                    <template #cell-platform="{ row }"
                        ><span class="tag">{{ row.platform || "-" }}</span></template
                    >
                    <template #cell-like_count="{ row }"
                        ><span class="metric like"
                            ><strong>{{ number(row.like_count) }}</strong></span
                        ></template
                    >
                    <template #cell-dislike_count="{ row }"
                        ><span class="metric dislike"
                            ><strong>{{ number(row.dislike_count) }}</strong></span
                        ></template
                    >
            ...
            <div class="section">
                ...
                <DataTable :columns="reportColumns" :rows="reviewReports" :loading="loading" empty-text="暂无待处理举报">
                    <template #cell-review="{ row }"
                        ><div class="book-title-cell">
                            <strong>#{{ row.review_id }} · {{ row.book_id }}</strong
                            ><small>{{ row.review_content }}</small>
                        </div></template
                    >
                    <template #cell-reason="{ row }"
                        ><span class="tag">{{ reportReasonLabel(row.reason) }}</span
                        ><br /><small>{{ row.details || "-" }}</small></template
                    >
            ...
            <div class="section">
                <div class="section-head">
                    <div>
                        ...
                        <p class="section-desc">
                            Bot 搜索无结果后用户点击提交产生；共 {{ number(requestSummary.total || 0) }} 次， {{ number(requestSummary.keywords || 0) }} 个关键词，{{ number(requestSummary.users || 0) }}
                            位用户。
                        </p>
                    </div>
                </div>
                <DataTable :columns="requestColumns" :rows="searchRequests" :loading="loading" empty-text="暂无缺书需求">
                    <template #cell-query="{ row }"
                        ><strong>{{ row.clean_query || row.query }}</strong></template
                    >
                    <template #cell-platform="{ row }"
                        ><span class="tag">{{ row.platform || "全部" }}</span></template
                    >
                    <template #cell-search_type="{ row }">{{ row.search_type || "search" }}</template>
                    <template #cell-status="{ row }"
                        ><span class="status-pill" :class="`status-${row.status || 'pending'}`">{{ requestStatusLabel(row.status) }}</span></template
                    >
                    <template #cell-submit_count="{ row }"
                        ><span class="metric like"
                            ><strong>{{ number(row.submit_count) }}</strong></span
                        ></template
                    >
            ...
            <div class="section">
                ...
                <DataTable :columns="crowdColumns" :rows="crowdRows" :loading="loading" empty-text="暂无众筹投票">
                    <template #cell-rank="{ row }"
                        ><strong>#{{ number(row.rank) }}</strong></template
                    >
                    <template #cell-book_id="{ row }"
                        ><code>{{ row.book_id }}</code></template
                    >
                    ...
                    >
                    <template #cell-supporter_count="{ row }"
                        ><span class="metric like"
                            ><strong>{{ number(row.supporter_count) }}</strong></span
                        ></template
                    >
        ...
        <FormModal :open="resolveOpen" title="完成缺书需求" :model="resolveForm" :fields="resolveFields" :textarea-fields="resolveTextareaFields" :checks="resolveChecks" :busy="resolveBusy" :error="resolveError" save-label="标记已缓存并通知" @close="resolveOpen = false" @save="saveResolution" />
    </section>
</template>
...
const tabs = computed(() => [
    ...
    ...(canModerate.value
        ? [
              { key: "reports", label: "待审举报", count: reviewReports.value.length },
              { key: "appeals", label: "待审申诉", count: reviewAppeals.value.length }
          ]
        : []),
    {
        key: "requests",
        label: "缺书需求",
        count: Number(requestSummary.value.pending || searchRequests.value.filter((row) => !row.status || row.status === "pending").length)
    },
...
async function saveResolution(form) {
    ...
    if (!row || resolveBusy.value) return;
    ...
    try {
        ...
        toast(`已标记缓存，通知成功 ${sent}，失败 ${failed}`, failed ? "info" : "success");
        await load();
    } catch (err) {
        resolveError.value = err.message || String(err);
    } finally {
