/**
 * [INPUT]: 依赖 QQ 官方 App Access Token、OpenAPI、Gateway 与富媒体分片上传协议，以及 Node Fetch/文件/摘要能力
 * [OUTPUT]: 对外提供 QQ API 客户端、Token 获取/凭据测试、文本/Markdown 内嵌键盘/文件发送和错误类型
 * [POS]: qq-bot 的唯一腾讯网络边界，集中处理鉴权刷新、超时、回复序号与本地导出文件上传
...
function createQqApiClient(options = {}) {
    ...
    async function uploadFile(target, filePath) {
        ...
        const prepare = await request(targetPath(target, "upload_prepare"), {
            method: "POST",
            body: JSON.stringify({
                file_type: 4,
        ...
        for (let index = 0; index < parts.length; index += 1) {
            ...
            if (!uploadResponse.ok) throw new QqApiError(`QQ 文件分片上传失败：HTTP ${uploadResponse.status}`, { status: uploadResponse.status });
            await request(targetPath(target, "upload_part_finish"), {
                method: "POST",
                body: JSON.stringify({
                    upload_id: prepare.upload_id,
                    part_index: Number(part.part_index ?? index),
        ...
        const merged = await request(targetPath(target, "files"), {
            method: "POST",
            body: JSON.stringify({ file_type: 4, srv_send_msg: false, upload_id: prepare.upload_id }),
            timeoutMs: Math.max(timeoutMs, 60000)
        });
    ...
    async function sendFile(target, filePath, reply = {}) {
        ...
        return request(targetPath(target, "messages"), {
            method: "POST",
            body: JSON.stringify({ msg_type: 7, media: { file_info: fileInfo }, ...nextReplyPayload(reply) }),
            timeoutMs: Math.max(timeoutMs, 60000)
        });
    }
...
module.exports = { MAX_FILE_BYTES, QqApiError, commandKeyboard, createQqApiClient, requestAppAccessToken, targetPath, testQqBotCredentials };
