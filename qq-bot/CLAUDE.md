...
`message-runtime.js`: QQ 主面板/帮助/签到/仅缓存搜索/分页/选书/模板库/组件工坊下载交互编排，以共享 PNG 实时预览、单行主动作、两列选项、最终导出耗时和分层键盘适配移动端，并在下载前执行范围及实时缓存双重校验。
`qq-api.js`: QQ OpenAPI 唯一网络客户端，封装 App Access Token、Markdown/图片长格式内嵌键盘、正确剥离代码围栏并保留普通 `#标签`/连字符的纯文本降级及 200 MB 内富媒体分片上传，对官方可重试错误按阶段退避且保持消息序号幂等。
...
- EPUB 成品与工坊选择先发送共享三联 PNG 预览，QQ 图片能力不可用时才回退为原文字面板；预览不建立任务、不扣费。
