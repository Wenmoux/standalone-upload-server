...
`epub-styles/`: EPUB 视觉插件与资源契约；生成器注册六种样式（含模板工坊和兼容样式），公开范围和可定制能力由服务层模板库统一声明。
`epub-custom-handlers.js`: Telegram EPUB 自定义回调领域层，处理底板/开关预览、返回模板库和带审计限流的持久导出确认。
`epub-studio-handlers.js`: Telegram 模板工坊回调领域层，以 4 字符无会话状态切换知识库组件并确认持久导出。
...
- 模板名称、公开状态和能力只在 `services/epub-style-config.js` 声明；Bot 自定义只覆盖注册表允许的字段，知识库片段在 CSS/XHTML/资源和测试完整前不得公开。
