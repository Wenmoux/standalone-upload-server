 ...
 * [OUTPUT]: 对外提供 PgBotClient 统一 HTTP 客户端、动态平台读取、QQ 运行配置及范围/实时缓存校验、来源感知签到、Bot 鉴权正文分页、管理员广播任务/收件人分页，以及用户显示名和邀请参数解析工具
 * [POS]: Telegram/QQ Bot 到 server-pg 的唯一业务数据访问层，封装鉴权、超时、缓存、幂等键与分页聚合，禁止直连 PostgreSQL
...
class PgBotClient {
    ...
    async qqBotConfig() {
        return this.request("/bot-api/config/qq-bot");
    }

    async qqBookAccess(bookId) {
        return this.request(`/bot-api/qq/books/${encodeURIComponent(bookId)}/access`);
    }
    ...
    async sign(telegramId, source = "telegram_bot") {
        return this.request(`/bot-api/users/${encodeURIComponent(telegramId)}/sign`, {
            method: "POST",
    ...
    async recordUserEvent(telegramId, type, detail = "", source = "telegram_bot") {
        return this.request(`/bot-api/users/${encodeURIComponent(telegramId)}/transactions`, {
            method: "POST",
            body: JSON.stringify({ currency: "copper", amount: 0, type, detail, source })
        });
    }
    ...
    async searchPlatforms() {
        return this.request("/reader-api/platforms");
    }
