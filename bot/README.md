...
书籍详情卡和书评列表都提供“写书评”按钮。私聊点击后直接发送下一条内容即可，不启用 Telegram 强制回复；群聊才使用 ForceReply，并且只接受对当前提示的回复，不会误把普通聊天当作书评。输入不符合长度规则时可原地重试，发送“取消”、`/cancel` 或点击“取消发布”都会清理草稿和原回复提示，退出输入状态；旧的 `/review 书号 内容` 仍保持兼容。
