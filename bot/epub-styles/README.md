...
| `style3` | 空门夜雨 | 完整原版字体、轻灰说明框、下划线分卷标题与居中粗宋章题 | 是 |
| `style4` | 丹青云卷 | 彩墨制作说明/信息/简介、双原字体、竖排分卷与无头图双色章题 | 是 |
| `crane` | 仙鹤章头 | 仙鹤头图、深色圆角标题条与旧版页面结构 | 否，仅兼容旧配置 |

“空门夜雨”以暮雨、旧门和大片留白概括参考版式的文学气质；名称不进入正文模板，也不绑定任何样例书名。

“丹青云卷”概括样式四的彩墨长屏、云山留白和卷轴分部视觉；参考 EPUB 的正文章头图含固定内容，因此资源、CSS 和正文 XHTML 都明确排除该图，其余有效页面结构与资源按样例复刻。

分卷页不是默认正文：生成器只在源章节中识别到非空真实分卷记录时调用 `renderVolume`。书籍没有分卷数据就不会插入“正文”或其他占位分卷。“空门夜雨”按参考 EPUB 把真实分卷写成正文流中的独立 XHTML：标题位于页面约 40% 处，左对齐并带底部横线，同时保留二级 NCX 目录；它不嵌入整页图片，也不声明全屏分卷属性。

样式一、样式三和样式四都包含原图/长屏双封面语义，因此保留原图作为书库缩略图，同时生成 `1080×2400` 的 `cover~slim.png`：模糊延展背景承接长屏空白，前景完整保留原封面。样式三不把封面加入内容目录；样式四按样例把封面列入目录，并在其后依次写入制作说明、书籍信息和简介。样式二遵循自己的标题页结构，不生成可见封面页或无用长屏资源。渲染失败时安全回退到原图。

四个样式的完整有效 CSS 与制作说明、书籍信息、简介、分卷、正文 XHTML 骨架都存放在 `assets/epub-templates/`。运行代码只按 `style1/style2/style3/style4` 读取文件并注入当前书籍数据，模板与文档不保存对应样例的书名或正文。
...
- 可选页面：`renderTitlePage`、`renderColophon`、`renderBookInfo`；只有实现并启用相应配置时才写入 EPUB。
...
- `style3` 打包参考 EPUB 的完整楷体和粗宋字体，不再打包数字字体、梅影或分卷图片。分卷使用 40% 顶距、1.3em 粗宋标题和 1px 黑色底线；正文使用 1.1em 居中粗宋章题，制作说明复刻 20% 顶距、1px 半透明边框、25px 圆角、轻灰阴影、60% 灰字和 10px 提示图。
- `style4` 打包参考 EPUB 的 `cc`/`llf` 原字体、三张 `1080×2400` 前置/分卷图和一张 `1080×750` 简介横幅。制作说明与书籍信息保持原容器和背景，简介保持横幅/竖排标签/双虚线，分卷保持彩墨长图与竖排卷名；`1080×750` 正文章头图不打包、不引用。
