...
- `style2`：样式二 · 原书复刻。直接迁移参考 EPUB 的原始 CSS、背景图、分卷图和三组正文章头；书名、作者、简介、章节、状态和封面使用当前书籍数据。
...
- 简介页标题。
- 是否嵌入样式头图。
- 样式二副标题、版本、来源、版权、阅读提示、字体族和追加 CSS。
- 样式一、样式二的五类页面实时预览与完整样式 CSS；分卷标签只展示版式，实际导出仅为非空真实分卷生成页面。
- 样式二标题页、制作说明、简介、分卷、章头和标志图片；页面同时显示原始推荐尺寸与当前图片尺寸。

配置保存到 `admin_config.bot_epub_style_config`。Bot 每次开始导出时从已有 `/bot-api/export-pricing` 响应读取配置，因此不增加额外请求。

样式二自定义图片保存在 `/config/epub-style2/`。Bot 点击 EPUB 后先展示样式一和样式二按钮，用户选择后才创建导出任务。
