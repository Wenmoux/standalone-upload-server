...
样式一和样式三各自的模板都包含原图/长屏双封面语义，因此这两种样式保留原图作为书库缩略图，同时生成 `1080×2400` 的 `cover~slim.png`：模糊延展背景承接长屏空白，前景完整保留原封面。样式三额外声明封面 `duokan-page-fullscreen` 与 Apple 字体显示选项，但不把封面加入内容目录；样式二遵循自己的标题页结构，不生成可见封面页或无用长屏资源。渲染失败时安全回退到原图。
