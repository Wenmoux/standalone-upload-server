...
`assets/`: 版本化内置资源，包含样式一头图、样式三梅影与从对应模板拆出的原始阅读提示图。
...
`style-one.js`: 江湖纸卷插件，读取 Style1 独立文件模板并声明长屏封面、竖排分卷、章头和可选内置头图。
`style-two.js`: 老二次元适配器，复用 `services/epub-style2-template.js` 的可配置模板和图片槽位。
`style-three.js`: 疏影横斜插件，读取 Style3 独立文件模板并声明长屏/全屏封面、Apple 字体、浅灰说明框、全屏真实分卷与居中章序。
