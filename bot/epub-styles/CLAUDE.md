...
`assets/`: 版本化内置资源，包含样式一头图及四种原始字体、样式三三种已去除样例文字的分部装饰图及三种字体、阅读提示图和仅供后台动态封面占位的梅影图。
...
`style-three.js`: 疏影横斜插件，读取 Style3 独立文件模板并声明长屏/全屏封面、原始字体、浅灰说明框、三种干净装饰图循环的全屏真实分卷与居中单行章题。
