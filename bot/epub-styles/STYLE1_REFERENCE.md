...
这些字体是针对原书裁剪的字形子集，不能保证覆盖其他书名、作者、简介和章节标题。江湖纸卷会直接打包原始字体以复现已覆盖字形，同时保留系统宋体、楷体和黑体回退链承接未覆盖的动态字符。
...
- `.intro-text` 使用原始楷体、不缩进、黑色文字；原 EPUB 的 `body` 与简介段落均不设置背景色。
...
- 头图容器、边距、字号、颜色和字体声明保持参考 EPUB 原值，不额外增加最大宽度或移动端覆盖规则。
- 制作说明改为项目通用文字，可在后台编辑或关闭。
- 封面继续使用书籍元信息中的 `cover`，不会套用样例封面；按 Style1 的双封面结构自动生成 `cover~slim.png`，也不会继承 Style3 专属的全屏 spine 与 Apple 显示声明。
- 网页 Reader 只复用主题、头图和章头，不生成封面、制作说明、简介或分卷独立页面。
