/**
 * [INPUT]: 依赖六个 EPUB 样式插件与 services/epub-style-config 的合法配置和模板库元数据
...
const styleStudio = require("./style-studio");
const { DEFAULT_EPUB_EXPORT_CONFIG, EPUB_STYLE_OPTIONS, normalizeEpubExportConfig } = require("../../services/epub-style-config");

const STYLES = new Map([styleOne, styleTwo, styleThree, styleFour, styleStudio, crane].map((style) => [style.id, style]));
